...
class DemoNode(Node):
    ...
    def __init__(self, name):
        ...
        self.segments = [Segment(up, [0, 0, 0], self.traj_time), Segment(self.waiting, [0, 0, 0], self.traj_time)]
        self.spline = None
        self.abort = False
    ...
    def newton_raphson(self, finalp, theta_guess):
        ...
        while np.linalg.norm(finalp - p) > 0.001:
            ...
            #N = np.identity(len(theta)) - J_inv @ J
            #secondary_task = alpha * (theta_guess - theta)
            #theta = theta + delta_theta + N @ secondary_task
        ...
        if len(self.segments) == 0:
            for obj in objs:
                ...
                if obj.type == 0:
                    ...
                    if q is not None:
                        self.segments.append(Segment(q, [0, 0, 0], self.traj_time))
                        self.segments.append(Segment(self.waiting, [0, 0, 0], self.traj_time))
                else:
                    ...
                    if q_center is not None:
                        ...
                        self.segments.append(Segment(q_center, [0, 0, 0], self.traj_time))
                        self.segments.append(Segment(q_p1, [0, 0, 0], self.tap_time))
                        self.segments.append(Segment(q_center, [0, 0, 0], self.tap_time))
                        self.segments.append(Segment(q_p2, [0, 0, 0], self.tap_time))
                        self.segments.append(Segment(q_center, [0, 0, 0], self.tap_time))
                        self.segments.append(Segment(self.waiting, [0, 0, 0], self.traj_time))
    ...
    def update(self):
        ...
        if np.linalg.norm(np.array(self.pcmd) - self.actpos) > self.contact_amount:
            ...
            self.segments = [Segment(self.waiting, [0, 0, 0], self.traj_time)]
