        ...
        if t < T1:
            ...
            if self.status == Modes.TOUCHINGDISC and self.contact == False:
                if t - self.point_sent < traj_time:
                    if self.ikin_calc is None:
                        startp = self.waitp
                        finalp = np.array([self.inputx, self.inputy, self.inputz])
                        self.ikin_calc = self.newton_raphson(startp, finalp, self.waiting)
                    qd, qddot = self.smooth_move(t - self.point_sent, traj_time, waiting, self.ikin_calc)
                    self.curr_qd = qd
                    tau   = self.gravity(self.actpos)
                ...
                if t - self.point_sent < traj_time:
                    ...
                    if self.ikin_calc is None:
                        ...
                        finalp = P1
                        self.ikin_calc = self.newton_raphson(startp, finalp, self.pose_qd)
                    qd, qddot = self.smooth_move(t - self.point_sent - traj_time - wait_time, tap_time, self.pose_qd, self.ikin_calc)
                    self.curr_qd = qd
                    tau   = self.gravity(self.actpos)
                    self.sendcmd(list(qd), list(qddot), tau)
                    ...
                    if self.ikin_calc is None:
                        ...
                        finalp = P2
                        self.ikin_calc = self.newton_raphson(startp, finalp, self.pose_qd)
                    qd, qddot = self.smooth_move(t - self.point_sent - traj_time - wait_time * 3 - tap_time * 2, tap_time, self.pose_qd, self.ikin_calc)
                    self.curr_qd = qd
                    tau   = self.gravity(self.actpos)
                    self.sendcmd(list(qd), list(qddot), tau)
